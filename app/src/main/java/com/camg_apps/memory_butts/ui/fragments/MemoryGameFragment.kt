...
import com.camg_apps.memory_butts.ui.EndGameActivity
...
class MemoryGameFragment : Fragment() {
  ...
  //  private var autoHide by Delegates.notNull<Boolean>()
        ...
        vmMemoryGame.lTime.observe(viewLifecycleOwner) { lTime ->
            lTime?.let {
                ...
                if (lTime == 0L) {
                    ...
                    goToEndGameActivity(0, amounPairs, vmMemoryGame.iMoves.value ?: 0)

                }
            }
        }

        vmMemoryGame.configure(requireContext())
        vmMemoryGame.newGame(imageViewList, cardViewList, amounPairs)
        vmMemoryGame.playGame(requireContext())
        {
            //Gana
            reproducirSonidoDePedoGanador()

            goToEndGameActivity((60 - vmMemoryGame.lTime.value!! ?: 0), amounPairs, vmMemoryGame.iMoves.value ?: 0)
    ...
    private fun goToEndGameActivity(lTime: Long, amounPairs: Int, movimientos: Int) {
        val bundle = Bundle()
        bundle.putInt("seconds", lTime.toInt())
        bundle.putInt("amountPairs", amounPairs)
        bundle.putInt("movimientos", movimientos)

        //primero regresar al fragment anterior
        //requireActivity().supportFragmentManager.popBackStack()
        findNavController().popBackStack()

       requireActivity().startActivity(Intent(requireContext(),
           EndGameActivity::class.java).putExtras(bundle));
    }
